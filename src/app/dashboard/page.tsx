...
import { getAuthorArticles, getAuthorStats } from '@/firebase/firestore/articles';
...
// --- Mock Data (Kept for fallback/notifications) ---
// articles array removed effectively by not using it
...
function Overview() {
  ...
  const [stats, setStats] = useState<any>({ totalArticles: 0, publishedCount: 0, draftCount: 0, totalViews: 0 });
  ...
  useEffect(() => {
    async function fetchData() {
      if (user && firestore) {
        try {
          const s = await getAuthorStats(firestore, user.uid);
          setStats(s);
          const a = await getAuthorArticles(firestore, user.uid);
          setRecentArticles(a.slice(0, 5)); // Top 5
        } catch (e) {
  ...
  return (
    <>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Articles" value={stats.totalArticles.toString()} trend="Lifetime" icon={FileText} delay={0.1} />
        <StatCard label="Published" value={stats.publishedCount.toString()} trend="Online" icon={Wifi} delay={0.2} />
        <StatCard label="Drafts" value={stats.draftCount.toString()} trend="In Progress" icon={PenTool} delay={0.3} />
        <StatCard label="Total Views" value={stats.totalViews > 0 ? stats.totalViews.toString() : "-"} trend="All Time" icon={Users} delay={0.4} />
      </section>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
        <div className="lg:col-span-2 space-y-6">
          <ChartWidget />
          <AssignmentsWidget articles={recentArticles} isLoading={isLoading} />
        </div>
...
function AssignmentsWidget({ articles, isLoading }: { articles: any[], isLoading: boolean }) {
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="bg-card border border-border rounded-lg shadow-sm overflow-hidden min-h-[300px]">
      <div className="flex items-center justify-between p-6 border-b border-border">
        <h3 className="font-serif text-lg font-bold">Recent Assignments</h3>
        <button className="text-xs font-mono text-muted-foreground hover:text-primary transition-colors">View All</button>
      </div>
      <div className="divide-y divide-border">
        {isLoading && <div className="p-8 text-center text-muted-foreground">Loading assignments...</div>}
        {!isLoading && articles.length === 0 && <div className="p-8 text-center text-muted-foreground">No assignments yet.</div>}
        ...
        {articles.map((article) => (
          <div key={article.id} className="group flex items-center justify-between p-4 hover:bg-foreground/5 transition-colors cursor-pointer">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h4 className="font-serif font-bold text-base group-hover:text-primary transition-colors line-clamp-1">{article.title}</h4>
                <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground font-mono uppercase tracking-wider">
                  <span>{article.category || 'General'}</span>
                  <span>•</span>
                  {/* Using publishDate if available, else 'Draft' */}
                  <span className={`flex items-center gap-1`}><Clock className="w-3 h-3" /> {article.status === 'published' ? 'Published' : 'Draft'}</span>
                </div>
              </div>
            </div>
