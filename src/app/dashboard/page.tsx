          ...
          <header className="sticky top-0 z-20 bg-background/80 backdrop-blur-md border-b border-border px-6 py-4 flex items-center justify-between transition-colors duration-500">
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-4">
                 ...
                 <div className="h-10 w-10 rounded-full bg-foreground text-background flex items-center justify-center font-serif font-bold text-lg border-2 border-transparent hover:border-primary hover:scale-105 cursor-pointer transition-all shadow-lg">
          ...
          <div className="p-6 md:p-8 max-w-[1800px] w-full mx-auto space-y-6 pb-20">
            ...
            {activeTab === 'overview' ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
                    ...
                    {/* Left Column: Editor & Drafts (2/3 width) */}
                    <div className="lg:col-span-2 space-y-6">
                        
                        {/* Visual Chart Area */}
                        <ChartWidget />

                        {/* Assignments / Drafts Table */}
                        <motion.div 
                        ...
                        <div className="divide-y divide-border">
                            {articles.map((article, i) => (
                                <div 
                                key={article.id} 
                                className="group flex items-center justify-between p-4 hover:bg-foreground/5 transition-colors cursor-pointer"
                                >
                                <div className="flex items-start gap-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground font-mono uppercase tracking-wider">
                                            ...
                                            <span>•</span>
                                            <span className={`flex items-center gap-1 ${article.deadline === 'Today, 4pm' ? 'text-orange-500 font-bold' : ''}`}>
                                                <Clock className="w-3 h-3" /> {article.deadline}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="space-y-6">
                        ...
                        className="bg-foreground text-background p-6 relative overflow-hidden rounded-lg shadow-lg group"
                        >
                        ...
                        <p className="font-serif text-xl leading-relaxed italic opacity-90 relative z-10">
                            "Journalism can never be silent: that is its greatest virtue and its greatest fault."
                        </p>
                        <div className="mt-4 flex items-center gap-2 opacity-60">
                            <div className="h-px w-8 bg-background"></div>
                            <p className="text-xs font-bold">Henry Anatole Grunwald</p>
                        </div>
                        </motion.div>
                    </div>
                </div>
    ...
    <aside className="w-[80px] md:w-64 bg-background/95 dark:bg-background/80 backdrop-blur-md border-r border-border flex flex-col sticky top-0 h-screen z-30 transition-all duration-300">
      ...
      <nav className="flex-1 py-6 px-3 space-y-1">
        {navItems.map((item) => (
          <button
            ...
            className={`
              w-full flex items-center gap-3 px-3 py-3 rounded-md transition-all duration-200 group relative overflow-hidden
              ${activeTab === item.id 
      ...
      <div className="p-4 border-t border-border space-y-2">
        ...
        <button 
            ...
            className={`w-full flex items-center gap-3 px-3 py-2 rounded-md transition-colors ${activeTab === 'profile' ? 'bg-foreground/5 text-foreground' : 'text-muted-foreground hover:text-foreground hover:bg-foreground/5'}`}
        >
...
function StatCard({ label, value, trend, icon: Icon, delay, isAlert }: any) {
  return (
    <motion.div
      ...
      transition={{ delay }}
      className={`
        p-6 border border-border rounded-lg relative group overflow-hidden
        ${isAlert ? 'bg-red-500/5' : 'bg-card'}
...
function ChartWidget() {
    return (
        <motion.div 
            ...
            className="bg-card border border-border p-6 relative overflow-hidden group min-h-[320px] flex flex-col rounded-lg shadow-sm"
        >
            <div className="flex justify-between items-center mb-8 relative z-10">
                ...
                <div className="flex gap-2">
                     {['1D', '1W', '1M'].map(t => (
                         <button key={t} className={`text-[10px] font-mono border border-border px-3 py-1 rounded transition-colors ${t === '1W' ? 'bg-foreground text-background' : 'hover:bg-muted'}`}>
                             {t}
                         </button>
                     ))}
                </div>
            ...
            <div className="flex-1 flex items-end justify-between gap-2 px-2 pb-2 relative z-10">
                ...
                <div key={i} className="flex-1 flex flex-col justify-end group/bar cursor-pointer h-full">
                    <div 
                        className="w-full bg-foreground/80 hover:bg-primary transition-all duration-300 relative rounded-t-sm"
                        style={{ height: `${h}%` }}
                    >
                        <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-foreground text-background text-[10px] font-bold px-2 py-1 rounded opacity-0 group-hover/bar:opacity-100 transition-opacity whitespace-nowrap z-20">
                            {h * 124} views
                        </div>
                        {/* Pattern overlay for tactile feel */}
...
function NotificationsWidget() {
    return (
        ...
        >
            ...
            <div className="space-y-6 relative">
                ...
                <div key={note.id} className="relative pl-6 group cursor-pointer">
                    ...
                    <p className="text-sm font-medium leading-snug text-foreground/90 group-hover:text-primary transition-colors">
                        <span className="font-bold text-foreground">{note.user}</span> {note.action}
                    </p>
            ...
            <button className="w-full mt-6 py-2 text-xs font-mono uppercase tracking-widest border border-border hover:bg-foreground hover:text-background transition-colors rounded">
                View All Comms
            </button>
        </motion.div>
    )
...
function EmptyState({ tab }: { tab: string }) {
    return (
        <motion.div 
            ...
            className="flex flex-col items-center justify-center h-[60vh] text-center border-2 border-dashed border-border rounded-xl bg-card/50"
        >
            <div className="p-6 bg-background rounded-full mb-6 shadow-sm">
                {tab === 'profile' && <User className="w-12 h-12 text-muted-foreground" />}
            ...
            <p className="text-muted-foreground max-w-md mx-auto mb-8">
                {tab === 'profile' 
                    ? "Your author profile settings, bio, and credentials would appear here."
                    : "This section is currently empty. Start writing or wait for data to populate."
                }
            </p>
            {tab !== 'profile' && (
                <button className="bg-foreground text-background px-6 py-2 rounded-md font-medium text-sm hover:bg-foreground/80 transition-colors">
                    Initialize {tab}
                </button>
            )}
        </motion.div>
