...
// Re-defining components from the large file provided by user, but now as separate components if needed
// For this single-file love, we keep them here.

const EditorBlock = ({ type, content, onContentChange, isDark, placeholder, onDelete }) => {
  return (
    <div className="relative group mb-6">
      <div 
        ...
        className={`editor-placeholder outline-none leading-relaxed transition-colors
          ${type === 'h2' ? 'text-3xl font-serif font-bold mb-4' : 
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: error.message || 'Could not save the article to the database.',
      });
    } finally {
    ...
    <div className={`min-h-screen pt-16 relative flex transition-colors duration-500 ${isDark ? 'bg-[#121212]' : 'bg-[#FDFBF7]'}`}>
      ...
      <div className={`flex-1 transition-all duration-500 ease-in-out ${isSidebarOpen && !isFocusMode ? 'lg:pr-[360px]' : 'pr-0'}`}>
        <div className="max-w-3xl mx-auto px-6 md:px-12 py-12">
          ...
          >
             {coverImageUrl ? (
               <Image src={coverImageUrl} alt="Cover image preview" layout="fill" objectFit="cover" />
             ) : (
              <div className="text-center">
                 <ImageIcon size={32} className={`mx-auto mb-2 ${isDark ? 'text-stone-600' : 'text-stone-400'}`} />
      ...
      >
        <div className={`h-14 border-b flex items-center px-2 ${isDark ? 'border-stone-800' : 'border-stone-200'}`}>
           {[{ id: 'checklist', icon: CheckCircle2, label: 'Tasks' }, { id: 'ai', icon: Bot, label: 'Co-Pilot' }, { id: 'assets', icon: Layout, label: 'Assets' }].map(tab => (
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        { label: "Tags selected", done: false }, // Placeholder
        ...
        <motion.div 
          ...
          className={`absolute bottom-0 left-0 right-0 h-16 border-t px-6 flex items-center justify-end z-10 transition-transform duration-500 ${isDark ? 'bg-[#181818] border-stone-800' : 'bg-white border-stone-200'}`}
        >
           <div className="flex items-center space-x-4">
              <button 
                onClick={handlePublish}
                disabled={isPublishing}
                className={`px-6 py-2 rounded-full text-sm font-bold tracking-wide uppercase text-white shadow-lg bg-orange-600 hover:bg-orange-700 transition-colors flex items-center space-x-2 disabled:bg-orange-800 disabled:cursor-not-allowed`}
              >
