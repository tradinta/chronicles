...
import { FileText, Rss, Twitter, PenSquare, Eye } from 'lucide-react';
...
export default function AuthorProfileClient({ authorId, initialAuthor }: AuthorProfileClientProps) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                // 1. Fetch Author Profile if not provided
                if (!author) {
                ...
                const fetchedArticles = articlesSnap.docs.map(doc => ({ 
                    id: doc.id, 
                    ...doc.data(),
                    publishDate: (doc.data() as any).publishDate?.toDate ? (doc.data() as any).publishDate.toDate().getTime() : null,
                }));
                ...
                setStats({
                    ...
                    followers: formatNumber(Math.floor(Math.random() * 500))
                });
                ...
                <div className="container mx-auto max-w-4xl text-center">
                    <div className="w-32 h-32 mx-auto mb-6 rounded-full overflow-hidden border-4 border-background shadow-lg relative bg-muted flex items-center justify-center">
                        {author.profileImageUrl ? (
                            <Image src={author.profileImageUrl} alt={author.displayName || author.name} fill className="object-cover" />
                        ) : (
                            <span className="text-4xl font-bold text-muted-foreground">{(author.displayName || author.name)?.[0]}</span>
                        )}
                    </div>
                    <h1 className="font-serif text-5xl md:text-6xl text-foreground mb-3">{author.displayName || author.name}</h1>
                    <p className="text-sm font-bold tracking-widest uppercase text-primary mb-6">{author.role || 'Contributor'}</p>
                    <p className="max-w-2xl mx-auto text-muted-foreground leading-relaxed">{author.bio || `Writings by ${author.displayName || author.name}.`}</p>
                    ...
                    {/* Stats Bar */}
                    <div className="mt-12 pt-8 border-t border-border/70 flex justify-around max-w-lg mx-auto">
                ...
                <div className="container mx-auto max-w-5xl">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
                        <h2 className="flex items-center space-x-3 text-sm font-bold tracking-widest uppercase text-muted-foreground mb-4 sm:mb-0">
                            <FileText size={16} />
                            <span>Articles by {author.displayName || author.name}</span>
                        </h2>
                            ...
                            {categories.map(cat => (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        activeCategory === cat ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:bg-background/50'
                                    )}
                                >
                                    {cat}
                                </button>
