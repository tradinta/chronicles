...
// Testimonial Card
const TestimonialCard = ({ quote, author, role, isDark }: { quote: string; author: string; role: string; isDark: boolean }) => (
...
const FeatureItem = ({ icon: Icon, title, desc, isDark }: { icon: any; title: string; desc: string; isDark: boolean }) => (
    <motion.div
        whileHover={{ y: -5 }}
...
const PricingCard = ({
    ...
    const getStyles = () => {
        switch (planType) {
            ...
            default: return { wrapper: '', accent: '', badge: null, btn: '' };
        }
    };
