...
import { Loader2, Camera, Quote, Video, Mic, Eye, EyeOff, Lock, Play, Pause } from 'lucide-react';
import Image from 'next/image';
import { Button } from '@/components/ui/button';

export default function OffTheRecordPublicPage() {
        ...
        <div className="p-6 md:p-8">
          ...
          <div className="mt-6 flex items-center justify-between border-t border-border pt-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground delay-reveal">
                {post.authorName}
              </span>
            </div>
...
function RedactedText({ text }: { text: string }) {
  ...
  // Split by ||redacted|| markers
  const parts = text.split(/\|\|(.*?)\|\|/g);

  return (
    <span>
      {parts.map((part, index) => {
        // If odd index, it was inside ||...||
        if (index % 2 === 1) {
    ...
    <div className="bg-zinc-900 text-white p-8 flex flex-col items-center justify-center min-h-[300px] relative overflow-hidden">
      ...
      <div className="flex items-end justify-center gap-1 h-32 w-full mb-8 opacity-50">
        {Array.from({ length: 40 }).map((_, i) => (
          <div
            key={i}
            className={`w-2 bg-green-500 transition-all duration-100 ${playing ? 'animate-pulse' : ''}`}
            style={{
      ...
      <button
        onClick={toggle}
        className="w-16 h-16 rounded-full bg-green-600 hover:bg-green-500 flex items-center justify-center transition-all hover:scale-105 shadow-xl shadow-green-900/20 z-10"
      >
