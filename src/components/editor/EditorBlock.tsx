...
interface EditorBlockProps {
    block: { id: number; type: string; content: string; source?: string; imageUrl?: string; caption?: string; };
    updateContent: (id: number, content: string) => void;
    isDark: boolean;
}

const EditorBlock = ({ block, updateContent, isDark }: EditorBlockProps) => {
    const [isFocused, setIsFocused] = useState(false);
    const { id, type, content, source, imageUrl, caption } = block;
    ...
    return (
      ...
      >
        ...
        <div className="relative">
          ...
          {type === 'h2' && (
            <div contentEditable suppressContentEditableWarning onBlur={e => updateContent(id, e.currentTarget.innerText)} className={`editor-placeholder text-3xl font-serif font-bold tracking-tight mb-4 outline-none ${isDark ? 'text-stone-100' : 'text-stone-900'}`} placeholder="Heading">{content}</div>
          )}
  
          {type === 'paragraph' && (
            <div contentEditable suppressContentEditableWarning onBlur={e => updateContent(id, e.currentTarget.innerText)} className={`editor-placeholder text-xl font-serif leading-[1.8] outline-none ${isDark ? 'text-stone-300' : 'text-stone-800'}`} placeholder="Start typing...">{content}</div>
          )}
  
          {type === 'quote' && (
            <div className={`my-8 pl-8 border-l-4 ${isDark ? 'border-primary' : 'border-primary'}`}>
               <div contentEditable suppressContentEditableWarning onBlur={e => updateContent(id, e.currentTarget.innerText)} className={`text-2xl font-serif italic outline-none mb-3 ${isDark ? 'text-stone-200' : 'text-stone-900'}`} placeholder="Quote text...">{content}</div>
               <div className="flex items-center space-x-2">
                  <div className="h-px w-8 bg-stone-400"></div>
                  <div contentEditable suppressContentEditableWarning className="text-xs font-bold uppercase tracking-widest opacity-70 outline-none" placeholder="Source Name">{source}</div>
               </div>
            </div>
          )}
          ...
          {type === 'image' && (
             <div className={`my-8 relative group rounded-lg overflow-hidden border transition-all ${isDark ? 'border-stone-800 bg-stone-900/50' : 'border-stone-200 bg-stone-50'}`}>
                {imageUrl ? <Image src={imageUrl} alt={caption || "Uploaded image"} layout="responsive" width={16} height={9} /> : (
                ...
                <input value={caption || ''} onChange={(e) => {/* update block caption state */}} className={`w-full p-3 text-xs bg-transparent border-t outline-none font-mono text-center ${isDark ? 'border-stone-800 text-stone-500' : 'border-stone-200 text-stone-400'}`} placeholder="Add caption..." />
             </div>
          )}
          ...
          {type === 'infobox' && (
            <div className={`my-8 p-6 rounded-lg border ${isDark ? 'bg-indigo-900/10 border-indigo-500/30' : 'bg-indigo-50 border-indigo-200'}`}>
                ...
                <div contentEditable suppressContentEditableWarning onBlur={e => updateContent(id, e.currentTarget.innerText)} className={`text-base leading-relaxed outline-none ${isDark ? 'text-stone-300' : 'text-stone-700'}`} placeholder="Add context or data points here...">{content}</div>
            </div>
          )}
        </div>
      </div>
